...
def get_sql_db_url_from_cloud_provider(
    cloud_provider: str, read_only: bool = False
) -> str:
...
def get_sql_db_url(read_only: bool = False) -> str:
    ...
    if config.DB_PASSWORD_SECRET_NAME:
        ...
        db_url: str = get_sql_db_url_from_cloud_provider(
            config.CLOUD_PROVIDER, read_only=read_only
        )
        return db_url
    # Running locally
    else:
