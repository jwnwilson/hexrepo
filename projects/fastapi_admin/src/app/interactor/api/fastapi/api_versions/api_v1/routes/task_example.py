...
from app.interactor.dependencies import get_task_adaptor
...
def start_task_celery(user_permission: UserPermissionCreateDTO) -> str:
    async_result = create_example_task.delay(user_dto=user_permission)
    return async_result.task_id


@router_v1.post("/task/serverless")
def start_task_serverless(user_permission: UserPermissionCreateDTO) -> TaskDTO:
    async_result: TaskPromise = create_example_task_serverless.delay(
        user_dto=user_permission
    )
