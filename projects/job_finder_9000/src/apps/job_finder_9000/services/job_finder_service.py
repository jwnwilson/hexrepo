...
class JobFinderService:
    ...
    def _initialize_agent(self):
        ...
        try:
            ...
            if not openai_api_key:
                logger.warning(
                    "OpenAI API key not found in settings. Agent will not be initialized."
                )
                return
    ...
    async def find_jobs_async(
        ...
        if use_cache:
            cache_key = self._get_cache_key(
                candidate_profile.email,
                {
                    ...
                    "locations": [
                        loc.city for loc in candidate_profile.preferred_locations
                    ],
                },
            )
            ...
            if cached_result:
                logger.info(
                    f"Returning cached job search results for {candidate_profile.email}"
                )
        ...
        try:
            ...
            if use_cache:
                cache.set(cache_key, result.model_dump(), self._get_cache_ttl())
    ...
    def get_job_recommendations(
        self, candidate_profile: CandidateProfile, max_results: int = 10
    ) -> List[JobPosting]:
        """
        ...
        try:
            ...
            recommendations = [
                job for job in result.jobs if job.match_score and job.match_score >= 70
            ]
    ...
    def analyze_candidate_market_position(
        ...
        try:
            ...
            result = self.find_jobs(
                candidate_profile=candidate_profile, max_results=50, include_remote=True
            )
            ...
            candidate_skills = {
                skill.name.lower() for skill in candidate_profile.skills
            }
            ...
            return {
                ...
                "recommendations": [
                    ...
                    if skill_gaps
                    else "Your skills are well-aligned with market needs",
    ...
    def clear_cache(self, candidate_email: Optional[str] = None):
        ...
        if candidate_email:
            ...
            pattern = f"job_search:{candidate_email}:*"  # noqa: F841
