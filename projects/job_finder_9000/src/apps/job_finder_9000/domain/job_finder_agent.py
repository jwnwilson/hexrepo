...
class CompanySearchResult(BaseModel):
    ...
    hiring_status: str = Field(
        ..., description="Current hiring status (active, limited, not hiring)"
    )
...
class CompanySearchResponse(BaseModel):
    ...
    companies: List[CompanySearchResult] = Field(
        ..., description="List of companies to search for jobs"
    )
    search_criteria: Dict[str, Any] = Field(
        ..., description="Criteria used for the search"
    )
...
class JobSearchResponse(BaseModel):
    ...
    total_jobs_found: int = Field(
        ..., description="Total number of jobs found for this company"
    )
...
class CandidateProfile(BaseModel):
    ...
    salary_expectation: Optional[int] = Field(
        None, description="Expected salary in USD"
    )
    job_preferences: Dict[str, Any] = Field(
...
class JobSearchRequest(BaseModel):
    ...
    salary_threshold: Optional[int] = Field(
        None, description="Minimum salary threshold"
    )
...
"""
    ...
    async def search_companies(
        self, company_agent_state: Dict[str, Any]
        ...
        context = {
            ...
            "remote_preference": [
                loc.remote_preference for loc in candidate.preferred_locations
            ]
            if candidate
            else ["hybrid"],
        ...
        try:
            with logfire.span("search_companies_prompt"):
...
Focus on finding high-quality, relevant job opportunities that would be attractive to the candidate based on their specific profile and preferences."""
    ...
    async def get_active_jobs(
        ...
        for company_name in companies:
            ...
            company_detail = next(
                (c for c in company_details if c.company_name == company_name), None
            )

            # Prepare search context for this company
            search_context = {
                "company_name": company_name,
                "company_location": company_detail.location
                if company_detail
                else "Unknown",
                "company_industry": company_detail.industry
                if company_detail
                else "Technology",
                "candidate_skills": [skill.name for skill in candidate.skills]
                if candidate
                ...
                "candidate_experience": candidate.experience_years if candidate else 0,
                "preferred_locations": [
                    loc.city for loc in candidate.preferred_locations
                ]
                if candidate
                else [],
            ...
            try:
                with logfire.span("get_active_jobs_prompt"):
...
class JobFinderOrchestrator:
    ...
    async def find_jobs(self, request: JobSearchRequest) -> JobSearchResult:
        ...
        with logfire.span("find_jobs"):
            ...
            if not locations:
                company_agent_state["include_remote"] = include_remote  # noqa: F821
            ...
            company_search_response = await self.company_finder.search_companies(
                company_agent_state
            )
            company_agent_state["companies"] = [
                company.company_name for company in company_search_response.companies
            ]
            company_agent_state["company_details"] = company_search_response.companies

            # Get active jobs from the high paying companies
            company_agent_state["active_jobs"] = await self.job_finder.get_active_jobs(
                company_agent_state
            )

            # Filter jobs by candidate skills and salary expectation
            company_agent_state["filtered_jobs"] = (
                filtered_jobs
            ) = await self.job_finder.filter_jobs(company_agent_state)
            ...
            summary = self._generate_search_summary(
                filtered_jobs, candidate, search_duration
            )
    ...
    def _generate_search_summary(
        self, jobs: List[JobPosting], candidate: CandidateProfile, duration: float
    ) -> str:
...
async def example_usage():
    ...
    candidate = CandidateProfile(
        ...
        preferred_locations=[
            ...
            Location(
                city="New York", state="NY", country="USA", remote_preference="hybrid"
            ),
        ],
    ...
    request = JobSearchRequest(  # noqa: F841
        candidate=candidate, max_results=5, include_remote=True, salary_threshold=80000
    )
