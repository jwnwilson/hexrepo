...
@router.post(
    "/search",
    response={200: JobSearchSuccessSchema, 400: ErrorSchema, 500: ErrorSchema},
)
def search_jobs(request, payload: JobSearchRequestSchema):
    ...
    """
    with logfire.span("search_jobs_request"):
        try:
            ...
            return 400, ErrorSchema(error="Validation Error", detail=str(e))
...
@router.post(
    "/recommendations",
    response={200: RecommendationsSuccessSchema, 400: ErrorSchema, 500: ErrorSchema},
)
def get_job_recommendations(request, payload: JobRecommendationsRequestSchema):
    ...
    try:
        ...
        recommendations = job_finder_service.get_job_recommendations(
            candidate_profile=candidate_profile, max_results=payload.max_results
        )
...
@router.post(
    "/market-analysis",
    response={200: MarketAnalysisSuccessSchema, 400: ErrorSchema, 500: ErrorSchema},
)
def analyze_market_position(request, payload: MarketAnalysisRequestSchema):
    ...
    try:
        ...
        analysis = job_finder_service.analyze_candidate_market_position(
            candidate_profile
        )
...
def clear_cache(request, candidate_email: str = None):
    ...
    try:
        ...
        message = (
            f"Cache cleared for candidate: {candidate_email}"
            if candidate_email
        ...
        return 200, CacheClearSchema(
            success=False, message=f"Failed to clear cache: {str(e)}", cleared_entries=0
        )
